...
<?php function seo_metabox(){
		...
		foreach($posts as $post):
			add_meta_box("seo-meta-container", __('SEO Options','dt_delicate'), "seo_settings", "{$post}", "normal", "high");
	...
	function seo_settings($args){ 
        ...
        <div class="custom-box">
            <div class="column one-sixth">
                <label><?php _e('Title','dt_delicate');?> </label>
            </div>
            <div class="column five-sixth last">      
                <input name="_seo_title" type="text" class="large"  value="<?php echo get_post_meta( $post->ID, '_seo_title', true );?>" />
                <p class="three-fourth note"> <?php _e('The title display in search engines is limited to 70 chars. If the SEO title is empty the title will be generated based on your title template in your SEO settings.','dt_delicate');?> </p>
             </div>
        </div>
        <div class="custom-box">
            <div class="column one-sixth">
            <label><?php _e('Description','dt_delicate');?> </label>
            </div>
            <div class="column five-sixth last">      
				...
				<p class="three-fourth note"> <?php _e('The meta description will be limited to 140 chars. If the meta description is empty the description <br> will be generated based on your meta description options in your SEO settings.','dt_delicate');?> </p>
             </div>
        </div>
        <div class="custom-box">
            <div class="column one-sixth">
            <label><?php _e('Keywords','dt_delicate');?> </label>
            </div>
            <div class="column five-sixth last">      
                <input name="_seo_keywords" type="text" class="large" value="<?php echo get_post_meta( $post->ID, '_seo_keywords', true );?>"/>
                <p class="note"> <?php _e('Add any additional comma separated keywords here.','dt_delicate');?> </p>
            </div>
        </div>
<?php
		wp_reset_postdata();
