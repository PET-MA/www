...
class MY_Portfolio_Widget extends WP_Widget {
	...
	function MY_Portfolio_Widget() {
		...
		$this->WP_Widget(false,IAMD_THEME_NAME.__(' Portfolio','dt_delicate'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo $this->get_field_id('_post_categories'); ?>">
			<?php _e('Choose the categories you want to display (multiple selection possible)','dt_delicate');?></label>
        ...
        <p><label for="<?php echo $this->get_field_id('_excerpt'); ?>"><?php _e('Display title only or title &amp; excerpt','dt_delicate');?></label>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('_enabled_image');?>" name="<?php echo $this->get_field_name('_enabled_image');?>"
	         <?php checked($_enabled_image); ?> /> <?php _e("Show Image",'dt_delicate');?></p>  

	    <p><label for="<?php echo $this->get_field_id('_post_count'); ?>"><?php _e('No.of posts to show:','dt_delicate');?></label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-portfolio-widget'><ul>";		
			 ...
			 endwhile;
			 	...
			 	echo "<li>".__('No Portfolio Entries found','dt_delicate')."</li>";
			 endif;
			 wp_reset_query();
	 	echo "</ul></div>";			 
		echo $after_widget;
