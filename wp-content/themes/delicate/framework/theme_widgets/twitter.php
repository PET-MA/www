...
class MY_Twitter extends WP_Widget {
	...
	function MY_Twitter() {
		...
		$this->WP_Widget(false,IAMD_THEME_NAME.__(' Twitter Widget','dt_delicate'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => __('Latest Tweets','dt_delicate'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','dt_delicate');?> 
		   <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" 
            type="text" value="<?php echo esc_attr($title); ?>" /></label></p>

        <p><label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:','dt_delicate');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:','dt_delicate');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','dt_delicate');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('Access Token Secret:','dt_delicate');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Enter your twitter username:','dt_delicate');?>
        ...
        <p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('How many entries do you want to show:','dt_delicate');?>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('exclude_replies');?>" name="<?php echo $this->get_field_name('exclude_replies');?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo $this->get_field_id('exclude_replies'); ?>"><?php _e('Exclude @replies','dt_delicate');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('time');?>" name="<?php echo $this->get_field_name('time');?>" 
			<?php checked($time); ?> /> <label for="<?php echo $this->get_field_id('time'); ?>"><?php _e('Show time of tweet','dt_delicate');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('time');?>" name="<?php echo $this->get_field_name('display_avatar');?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo $this->get_field_id('display_avatar'); ?>"><?php _e('Show user avatar','dt_delicate');?></label></p>		
<?php
	}
		...
		echo $before_widget;			
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					...
					echo '<li>'.__('No public Tweets found','dt_delicate').'</li>';
				}
				echo "</ul>";
			}
		echo $after_widget;
