...
class MY_Recent_Pages extends WP_Widget {
	...
	function MY_Recent_Pages() {
		...
		$this->WP_Widget(false,IAMD_THEME_NAME.__(' Pages','dt_delicate'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo $this->get_field_id('_pages'); ?>">
			<?php _e('Choose the pages you want to display (multiple selection possible)','dt_delicate');?></label>
            <select class="widefat" id="<?php echo $this->get_field_id('_pages').'[]';?>" 
            	name="<?php echo $this->get_field_name('_pages').'[]';?>" multiple="multiple">
                <option value=""><?php _e("Select Page",'dt_delicate');?></option>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-pages-widget'><ul>";
			 ...
			 endwhile;
				...
				echo "<li><h6>".__('No Pages found','dt_delicate')."</h6></li>";			 
			 endif;
			 wp_reset_query();
	 	echo "</ul></div>";			 
		echo $after_widget;
